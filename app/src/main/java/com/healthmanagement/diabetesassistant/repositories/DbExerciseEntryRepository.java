...
public class DbExerciseEntryRepository implements IExerciseEntryRepository
{
	...
	public void create( ExerciseEntry item )
	{
		...
		contentResolver.insert( DAContentProvider.EXERCISE_ENTRIES_URI, this.putContentValues( item ) );
	...
	public ArrayList<ExerciseEntry> readAll( String userName )
	{
		...
		Cursor cursor = contentResolver.query( DAContentProvider.EXERCISE_ENTRIES_URI,
	...
	public void update( int id, ExerciseEntry entry )
	{
		...
		contentResolver.update( DAContentProvider.EXERCISE_ENTRIES_URI,
	...
	{
		contentResolver.delete( DAContentProvider.EXERCISE_ENTRIES_URI,
				DB.KEY_ID + "=?",
				new String[]{ String.valueOf( id ) } );
