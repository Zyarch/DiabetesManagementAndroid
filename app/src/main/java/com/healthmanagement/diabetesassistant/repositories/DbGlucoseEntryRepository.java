...
public class DbGlucoseEntryRepository implements IGlucoseEntryRepository
{
	...
	private Uri uri = DAContentProvider.GLUCOSE_ENTRIES_URI;
	...
	{
		Cursor cursor = contentResolver.query( DAContentProvider.GLUCOSE_ENTRIES_URI,
				null,
				DB.KEY_REMOTE_ID + "=?",
				new String[]{ id },
				null );
...
//		entry.setId( cursor.getInt( cursor.getColumnIndex( DB.KEY_ID ) ) );
	...
	public void setAllSynced()
	{
		...
		contentResolver.update( DAContentProvider.GLUCOSE_ENTRIES_URI,
